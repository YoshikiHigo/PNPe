	...
	public static String normalize(final String text) {
		...
		while (true) {
			...
			final int doubleQuotationEndIndex = normalizedText.indexOf("\"",
					doubleQuotationStartIndex + 1);

			if (doubleQuotationStartIndex < endIndex
					&& endIndex < doubleQuotationEndIndex) {
				endIndex = normalizedText.indexOf("$$",
	...
	private void generateText() {
		...
		if (this.core instanceof StatementInfo) {
			...
			case Assert: {
				...
				if (1 < core.getExpressions().size()) {
					text.append(" : ");
					final ProgramElementInfo message = core.getExpressions()
							.get(1);
